...
def bantuan(commands,role,status): # bukan help() karena help() adalah fungsi built-in, plesetin dikit deh
    asciiart()

    availible_commands = []
    for i in commands:
        if i[2] == 0 or i[2] == role:
            if i[3] == 0 or i[3] == status:
                availible_commands += [i]
    for i in range(length(availible_commands)):
        print(str(i+1) + ". " + availible_commands[i][0] + " - " + availible_commands[i][1])


# def bantuan(): # bukan help() karena help() adalah fungsi built-in, plesetin dikit deh
#     asciiart()
